...
def train_data_generate(img_path_and_js, out_file, yolo_label_file, json_out_file, data_file, num, page_index):
    for path in img_path_and_js:
        ...
        for angle in range(24):
            num = crop_json(data_file, js, img_path, angle * 15, 640, 640, 320, 320, out_file, yolo_label_file, num,
                            page_index, json_out_file)
        # num = crop_json(data_file, js, img_path, 0, 640, 640, 320, 320, out_file, yolo_label_file, num,
        #                 page_index, json_out_file)
        page_index += 1
...
if __name__ == "__main__":
    # src_dir: 存储原数据的路径
    # out_image_file: 切割后图片的存储路径
    # out_label_file: 切割后yolo格式标注数据的存储路径
    # json_out_file: 切割后各图片,各角度label的原格式数据存储路径（中转数据，验证图片标注点用）
    # data_file: 将原数据存储格式转为函数用格式后的数据路径

    src_dir = "D:/pycharm/Medical-images/MedCV/dataset/original_testset"
    out_image_file = "D:/pycharm/Medical-images/MedCV/dataset/paper_datasets/test/images"
    out_label_file = "D:/pycharm/Medical-images/MedCV/dataset/paper_datasets/test/labels"
    json_out_file = "D:/pycharm/Medical-images/MedCV/dataset/paper_datasets/temp_json"
    data_file = "D:/pycharm/Medical-images/MedCV/dataset/paper_datasets/test/data_file"
    main(src_dir, out_image_file, out_label_file, json_out_file, data_file)
