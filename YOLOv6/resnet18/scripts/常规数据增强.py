...
def random_augument(image_dir, out_img_file, target_count):
    # 设定各数据增强方法的触发概率
    pbb_gauss_noise = 0.0
    pbb_bs_noise = 0.0
    pbb_random_Color = 0.0
    pbb_random_rotate = 1.0
    pbb_random_mirror = 1.0
    pbb_image_merge = 0.0
    image_merge_flag = 0
...
def augument(image_dir, out_img_file):
    count = 0
    images = []
    # labels = []
    img_len = len(os.listdir(image_dir))
    ...
    for img in images:
        for angle in range(0, 360, 90):
            new_img1 = copy.deepcopy(random_rect_rotate(img, angle))
            if angle < 180:
                for angle1 in range(0, 270, 90):
                    if angle1 != 180:
                        new_img2 = copy.deepcopy(mirror(new_img1, angle1))
                    else:
                        new_img2 = copy.deepcopy(new_img1)
...
if __name__ == "__main__":
    ...
    image_dir = "/ResNet50-master/cell_data/test/Square_test_sp_kp"
    # label_dir = "D:/pycharm/Medical-images/MedCV/dataset/paper_datasets/datasetC/50000/train/labels"
    out_img_file = "/resnet18/dataset_balance/test/Square_test_sp_kp"
    # out_label_file = "D:/pycharm/Medical-images/MedCV/dataset/paper_datasets/datasetC/50000/train/augument_labels"
    target_count = 2500

    # random_augument(image_dir, out_img_file, target_count)
    # augument(image_dir, out_img_file)
    # test()
    # for image in os.listdir(image_dir):
    ...
    # 标注可视化
    image_path = '/dataset/paper_datasets/datasetC/50000/train/images1/augument_7_180_0.jpg'
    label_path = '/dataset/paper_datasets/datasetC/50000/train/labels1/augument_7_180_0.txt'
    out_file = '/dataset/paper_datasets/datasetC/50000/train'
    img = cv2.imread(image_path)
    label = []
