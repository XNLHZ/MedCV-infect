...
def data_divide(images, labels, num, img_out_file, label_out_file, train_prop=0.8, val_prop=0.1, test_prop=0.1):
    img_index = []
    for i in range(num):
    ...
    for index in img_index:
        source_img = images + "img (" + str(index) + ").jpg"
        source_label = labels + "img (" + str(index) + ").txt"
        if count < train_stop:
            if not os.path.exists(destination_img + "/train"):
                os.makedirs(destination_img + "/train")
            if not os.path.exists(destination_label + "/train"):
                os.makedirs(destination_label + "/train")
            ...
            if not os.path.exists(destination_img + "/val"):
                os.makedirs(destination_img + "/val")
            if not os.path.exists(destination_label + "/val"):
                os.makedirs(destination_label + "/val")
            ...
            if not os.path.exists(destination_img + "/test"):
                os.makedirs(destination_img + "/test")
            if not os.path.exists(destination_label + "/test"):
                os.makedirs(destination_label + "/test")
...
images = "D:/pycharm/Medical-images/MedCV/dataset/cloth_YOLO_data/resized_data/cutted/img/"              # 存放图片数据的路径
labels = "D:/pycharm/Medical-images/MedCV/dataset/cloth_YOLO_data/resized_data/cutted/label/"              # 存放标注数据的路径
img_out_file = "D:/pycharm/Medical-images/MedCV/dataset/cloth_YOLO_data/divide_data/cutted/images"     # 输出划分后图片的路径
label_out_file = "D:/pycharm/Medical-images/MedCV/dataset/cloth_YOLO_data/divide_data/cutted/labels"   # 输出划分后标注的路径
num = 1445                                        # 图片总数
data_divide(images, labels, num, img_out_file, label_out_file, 0.9, 0, 0.1)
